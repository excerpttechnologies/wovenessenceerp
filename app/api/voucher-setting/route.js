...
const VOUCHER_SETTING = { screen: SCREENS.VOUCHER_SETTING, label: 'voucher settings' };
...
export async function GET(req) {
  ...
  const denied = await screenDenial({
    session, ...VOUCHER_SETTING, action: PERM.READ, businessId: sp.get('business'),
  });
...
export async function POST(req) {
  ...
  let gate = await screenDenial({
    session, ...VOUCHER_SETTING, action: PERM.UPDATE, businessId: body.business,
  });
  if (gate) {
    gate = await screenDenial({
      session, ...VOUCHER_SETTING, action: PERM.CREATE, businessId: body.business,
    });
  }
