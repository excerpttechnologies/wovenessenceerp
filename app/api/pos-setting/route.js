...
/* Permission gate for this screen.
   ...
   Nothing else reads this route. The till does not load it -
   components/PosTill.jsx fetches counters, customers, items and holds, not
   this - so gating it does not affect billing. */
const POS_SETTING = { screen: SCREENS.POS_SETTING, label: 'POS settings' };
...
export async function GET(req) {
  ...
  const denied = await screenDenial({
    session, ...POS_SETTING, action: PERM.READ, businessId: sp.get('business'),
  });
...
export async function POST(req) {
  ...
  let gate = await screenDenial({
    session, ...POS_SETTING, action: PERM.UPDATE, businessId: body.business,
  });
  if (gate) {
    gate = await screenDenial({
      session, ...POS_SETTING, action: PERM.CREATE, businessId: body.business,
    });
  }
