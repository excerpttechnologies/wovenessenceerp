...
/* Permission gate for this screen.
   ...
   Nothing else reads this route - the till and every other form pick a
   method through /api/options?ref=paymentmethod - so gating here closes
   the master without touching a single screen that merely takes payment.
   */
...
export async function GET(req) {
  ...
  const denied = await screenDenial({
    session, ...PAYMENT_METHOD, action: PERM.READ, businessId: sp.get('business'),
  });
...
export async function POST(req) {
  ...
  /* Ahead of validate(), so a refused method comes back as 403 "not allowed"
     rather than 422 "your form is wrong". */
  const denied = await screenDenial({
    session, ...PAYMENT_METHOD, action: PERM.CREATE, businessId: body.business,
  });
  if (denied) return json({ error: denied.message, code: denied.code }, 403);
