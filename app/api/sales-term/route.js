...
/* Permission gate for this screen.
   ...
   /api/options?ref=sales/master/term, lib/refLabels.js resolves a stored id
...
export async function GET(req) {
  ...
  const denied = await screenDenial({
    session, ...SALES_TERM, action: PERM.READ, businessId: sp.get('business'),
  });
...
export async function POST(req) {
  ...
  const denied = await screenDenial({
    session, ...SALES_TERM, action: PERM.CREATE, businessId: body.business,
  });
  if (denied) return json({ error: denied.message, code: denied.code }, 403);
